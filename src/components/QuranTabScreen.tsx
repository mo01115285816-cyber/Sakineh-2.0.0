...
const QuranTabScreen = React.memo(function QuranTabScreen({
  onBack,
  onHideNavChange,
}: Props) {
  // Master Mode: "listening" (audio, broadcast, reciters) or "reading" (gate layout)
  const [quranMode, setQuranMode] = useState<"listening" | "reading">(
    "listening",
  );

  // Navigation stack: "reciters" | "surahs" | "player"
  const [currentScreen, setCurrentScreen] = useState<"reciters" | "surahs">(
    "reciters",
  );
  ...
  // Sleep Timer state
  const [timerMinutesRemaining, setTimerMinutesRemaining] = useState<
    number | null
  >(null);
  ...
  const playSurah = useCallback(
    (surahId: number, allSurahs: number[]) => {
      const {
        ...
        if (audioRef.current) {
          try {
            ...
            if (
              ...
              currentBlobUrl.current.startsWith("blob:")
            ) {
  ...
  const handleNext = useCallback(() => {
    const {
      ...
      playingSurahId: currentPlayingId,
  ...
  const handlePrev = useCallback(() => {
    const { playlist: currentPlaylist, playingSurahId: currentPlayingId } =
      stateRef.current;
    if (currentPlaylist.length === 0 || currentPlayingId === null) return;
    const currentIndex = currentPlaylist.indexOf(currentPlayingId);
    if (currentIndex === -1) return;
            ...
            >
              ...
              <div className="flex items-center gap-2.5 flex-1 min-w-0">
                {currentPlayingRadio !== null ? (
                  <div
                    className={`relative w-[42px] h-[42px] rounded-full text-white flex items-center justify-center text-[12.5px] font-bold shrink-0 shadow-[0_4px_10px_rgba(0,0,0,0.12)] ring-2 ring-[#fdfcfb] overflow-hidden ${
                  ...
                  >
                    ...
                    <span className="relative z-10 drop-shadow-sm">
                      {currentPlayingRadio.logoType === "sba" ? "بث" : "مباشر"}
                    </span>
                  </div>
                ) : (
                  ...
                  <div className="relative w-[42px] h-[42px] rounded-full text-white flex items-center justify-center text-[16px] font-bold shrink-0 shadow-[0_4px_10px_rgba(0,0,0,0.12)] ring-2 ring-[#fdfcfb] overflow-hidden bg-gradient-to-br from-[#e0b06b] via-[#c49a62] to-[#a37c48]">
                    <div className="absolute inset-0 bg-gradient-to-b from-white/25 to-transparent opacity-60 rounded-full"></div>
                    <span className="relative z-10 drop-shadow-sm">
                      {selectedReciter.name.trim().charAt(0)}
                    </span>
                  </div>
                )}
                ...
                <div className="text-right flex flex-col justify-center pb-[1px] overflow-hidden flex-1 max-w-[200px] xs:max-w-[220px] sm:max-w-[250px]">
                  <h4 className="text-[14px] font-bold text-[#2b1a10] leading-tight font-sans mb-[1px] truncate w-full">
                    {currentPlayingRadio !== null
