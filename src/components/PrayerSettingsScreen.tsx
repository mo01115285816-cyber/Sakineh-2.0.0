...
interface PrayerSettingsScreenProps {
  ...
  onClose: () => void;
  onSave: (pref: SinglePrayerPreference) => void;
  // Dynamic dates to calculate secondary timings accurately
  fajrDate?: Date;
  sunriseDate?: Date;
  maghribDate?: Date;
}
...
export const PrayerSettingsScreen = React.memo(function PrayerSettingsScreen({
  ...
  // Load saved prefs or fallback to defaults
  const initialPrefs = useMemo<SinglePrayerPreference>(() => {
    try {
      ...
      console.warn("Failed to read prayer settings from localStorage", e);
    }
  ...
  }, [prayerId, prayerDisplayName]);

  const [currentPrefs, setCurrentPrefs] =
    useState<SinglePrayerPreference>(initialPrefs);
  const [showUnsavedModal, setShowUnsavedModal] = useState(false);

  // Sync state if initial changes (e.g. prayerId changes)
  useEffect(() => {
    setCurrentPrefs(initialPrefs);
  }, [initialPrefs]);

  const isDirty = useMemo(() => {
    return JSON.stringify(currentPrefs) !== JSON.stringify(initialPrefs);
  ...
  const handleSaveAndClose = useCallback(() => {
    ...
    try {
      localStorage.setItem(
        `app_prayer_pref_${prayerId}`,
        JSON.stringify(currentPrefs),
      );
    } catch (e) {
      console.warn("Failed to persist prayer preference:", e);
    }
    onSave(currentPrefs);
    onClose();
  }, [currentPrefs, prayerId, onSave, onClose]);
  ...
  const handleToggleEnabled = useCallback((val: boolean) => {
    setCurrentPrefs((prev) => ({ ...prev, enabled: val }));
  }, []);

  const handleSelectMode = useCallback((mode: PrayerNotificationMode) => {
    setCurrentPrefs((prev) => ({ ...prev, mode }));
  }, []);

  const handleSelectMuezzin = useCallback((muezzinId: string) => {
    setCurrentPrefs((prev) => ({ ...prev, selectedMuezzinId: muezzinId }));
  }, []);

  // Concised title: dynamically reflects selected prayer (strictly 2 words)
  ...
  return (
    ...
    >
      ...
      <div className="fixed top-6 left-5 right-5 flex items-center justify-between z-45 pointer-events-none">
        {/* Right Element: Concise Title Capsule */}
      ...
      <div className="space-y-4 relative z-10 pt-4">
        {/* ── TOP TOGGLE CARD: إشعارات الصلاة ── */}
        ...
        >
          <div className="flex flex-col divide-y divide-[#e6dccf]/30">
            ...
            {/* Item 2: صوت الأذان & الأذان الكامل (المنزلق) */}
            <div className="flex flex-col">
              ...
              {/* الأذان الكامل (يظهر فقط عند تفعيل صوت الأذان) */}
        ...
        {/* ── EXPANDABLE MUEZZIN SELECTION SECTION ── */}
        <AnimatePresence>
          {isMuezzinVisible && (
            <motion.div
            ...
            >
              <MuezzinSelectorSection
                ...
                onSelectMuezzin={handleSelectMuezzin}
              />
            </motion.div>
          )}
        </AnimatePresence>
      ...
      {/* Unsaved Changes Warning Modal Interceptor */}
      <UnsavedChangesModal
